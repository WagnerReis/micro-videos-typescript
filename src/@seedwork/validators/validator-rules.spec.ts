...
describe("ValidationRules Unit Tests", () => {
  ...
  test("required validation rule", () => {
    ...
    arrange.forEach(item => {
      expect(() => ValidatorRules.values(item.value, item.property).required()).toThrow(
        new ValidationError(item.messageError)
      );
    });
    ...
    arrange.forEach(item => {
      expect(() => ValidatorRules.values(item.value, item.property).required()).not.toThrow(
        new ValidationError(item.messageError)
      );
    });
  });
